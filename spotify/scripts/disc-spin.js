const easeDiscSpin = (disc, targetRate) => {
  const [spinAnimation] = disc.getAnimations().filter(
    (animation) => animation.animationName === "spin",
  );
  ...
  cancelAnimationFrame(disc.spinTweenFrame);
  ...
  const tick = (now) => {
    ...
    if (progress < 1) {
      disc.spinTweenFrame = requestAnimationFrame(tick);
    } else if (shouldPause) {
      spinAnimation.pause();
    }
  };
